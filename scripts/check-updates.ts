#!/usr/bin/env tsx
/**
 * Argentine Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.argentina.gob.ar/normativa';
const PORTAL_NAME = 'InfoLEG';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/argentine-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Argentine Law MCP — Data Freshness Check');
