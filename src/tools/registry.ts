/**
 * Tool registry for Argentine Law MCP Server.
...
import { getArgentineImplementations, type GetArgentineImplementationsInput } from './get-argentine-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Federal Register of Legislation (Argentine Government, Office of Parliamentary Counsel). ' +
    ...
    'Call this FIRST when you need to understand what Argentine legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Argentine statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Argentine statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Argentine legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Argentine statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Argentina?" ' +
  ...
  },
  {
    ...
    description:
      'Format an Argentine legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Argentine statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that an Argentine statute references or aligns with. ' +
      'Argentina is not an EU member but Argentine laws may reference EU directives/regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Argentine statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_argentine_implementations',
    description:
      'Find all Argentine statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Argentine statutes. ' +
      'Note: Argentina does not transpose EU law but may reference or align with EU frameworks voluntarily.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Argentine legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_argentine_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Argentine legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Argentine statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for an Argentine statute or provision. ' +
      ...
      'Note: Argentina is not bound by EU law; this checks voluntary alignment and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Argentine statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_argentine_implementations':
          result = await getArgentineImplementations(db, args as unknown as GetArgentineImplementationsInput);
          break;
