/**
 * get_argentine_implementations — Find Argentine statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetArgentineImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface ArgentineImplementationResult {
  document_id: string;
...
export async function getArgentineImplementations(
  db: InstanceType<typeof Database>,
  input: GetArgentineImplementationsInput,
): Promise<ToolResponse<ArgentineImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as ArgentineImplementationResult[];
